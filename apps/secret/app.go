...
import (
	...
	"github.com/Murphychih/cmdb/common/crypto/aescbc"
...
func (s *CreateSecretRequest) EncryptAPISecret(key string) error {
	...
	cipherText, err := aescbc.Encrypt([]byte(s.ApiSecret), []byte(key))
	if err != nil {
		return err
	}
...
func (s *CreateSecretRequest) DecryptAPISecret(key string) error {
	...
	planText, err := aescbc.Decrypt([]byte(cipherText), []byte(key))
	if err != nil {
		return err
	}
